...
would be defined by six or eight integers on the connection card. The angle й
...
н, is described by the equation:

               N     n           N     n*
   ur (н)  =   -   ur  cos nн +  -   ur   sin nн                    (1)
              n=0               n=0
...
symmetric motion with respect to the н = 0 plane. The second sum with the
...
system (r, н, z) and their applied loads must be described in this coordinate
...
are given in the r, н, and z directions. The value of a harmonic load Fn is
...
orthotropic axes in the (r,z) plane is specified by the angle й. The
...
cylindrical coordinate system (r, н, z). The rotational degrees of freedom
...
from the grid point are measured along the axes (r, й, z or p, й, н) defined
...
parallel to those r, й, z or p, й, н axes at that grid point. The CONM2
...
and the octahedral stress
                      ...
                      2            2            2       2       2       2 1/2
   +o  = 1/3[(+x - +y)  + (+y - +z)  + (+z - +x)  + 6зyz  + 6зzx  + 6зxy )
...
transformation derived from real eigenvalue analysis of the set ua. These are:
   ...
   оo rigid body (zero frequency) modal coordinates,

   оf finite frequency modal coordinates,

   оi = оo + оf, the set of all modal coordinates.

   One vector set is defined that combines physical and modal coordinates.
That set is uh = оi + ue, the set used in dynamic analysis by the modal
method.
...
cards, й is defined on a DPHASE card and з is defined on a DELAY card. The
RLOAD2 card defines a frequency-dependent load of the form 

                    i{н(f)+й-2+fз}
   {P(f)}  = {AB(f)e              }                                 (2)

where A is defined on a DAREA card, B(f) and н(f) are defined on TABLEDi
cards, й is defined on a DPHASE card, and з is defined on a DELAY card. The
...
where A is defined on a DAREA card, з is defined on a DELAY card, and F(t-з)
...
where t~  =  t - T1  - з, and A and з are defined as above. The coefficients
...
bulk data card).
   ...
   [Bdd]  =  [Bdd1] + [Bdd2] + (g/ю3)[Kdd1] + (1/ю4)[Kdd4]          (5)

   [Mdd]  =  [Mdd1] + [Mdd2]                                        (6)

where ю3 is the radian frequency at which the term (g/ю3)[Kdd1] produces the
same magnitude of damping as the term ig[Kdd1] in frequency response analysis,
and ю4 is the radian frequency at which the term (1/ю4)[Kdd4] produces the same
...
lower frequencies. Therefore, the quantities ю3 and ю4 are frequently selected
by you to be at the center of the frequency range of interest. A small value
of g/ю3 is frequently useful to insure stability of higher modes in nonlinear
transient analysis. You specify the values of ю3 and ю4 on PARAM cards W3 and
W4 (see PARAM bulk data card). If ю3 and ю4 are omitted, the corresponding
terms are ignored.
...
are as follows:
   ...
   1. The fluid is described by points in space and finite element
      ...
      describing the pressure function, P(н), in the following equation:

                   N   n          N   n*
      P(н) =  po + -  p  cos nн + -  p   sin nн    0< N <100
                   n=1            n=1
      ...
      of pressure at the angle, н, given in the above equation. The output of
      free surface displacements normal to the surface (FREEPT) are also
      available at specified angles, н. The Case Control card option "AXISYM =
...
symmetry is assumed at н = 0.0 and the second plane of symmetry is assumed at
н = 360 degrees/M where M is an integer specified on the card. Also specified
...
values on PARAM Bulk Data cards for:
   ...
   EPSHT (real)    о convergence parameter (default .001).
...
Data cards, and they are selected by TEMP(MATERIAL) in Case Control.
   ...
   1. Normal convergency:  оT < EPSHT, where оT is the per unit error estimate
      of the temperatures calculated.

   2. Number of iterations > MAXIT.

   3. Unstable: |ю1| < 1 and the number of iterations > 3, where ю1 is a
      stability estimator.
...
8.4. Error estimates оp, ю1, and оT for all iterations may be output with the
Executive Control card DIAG 18, where оp is the ratio of the Euclidian norms
...
circumference with the first slot located at н = 0 degrees. The parameter N
...
modes. The pressure is assumed to have the following distribution
   ...
   p(r,z,н)  =  p(r,z) cos Nн

   If N = 0 the breathing and longitudinal modes will result. If N = 1 the
pressure at н = 180 degrees will be the negative of the pressure at н = 0
degrees. If N = 2, the pressures at н = 90 degrees and н  = 270 degrees will
be the negative of that at н = 0 degrees. Values of N larger than M/2 have no
significance.
...
stored in the "CSTM" item for that pseudostructure. 
       ...
       INTERNAL      CSTM ID           --------юCOORDINATESю-----------
...
default value for this ratio is 1.0. The attachment flexibilities, Dz, Dйx,
and Dйy, allow for smoothing, but usually all values are taken to be zero. An
...
parameter is to compute the flutter index, by choosing VREF =bwй * sqrt(u).
...
defining the variable, n, that is, юks; юkc,S; юks,A; and юkc,A. In the right
...
where P1 is the current property value and з is an iteration factor with a
...
taken for all loading conditions. Values of з smaller than unity limit the
...
where о is a user-supplied convergence limit.
...
stressed design capability: 
   ...
   3. The property optimization parameters must be specified on the bulk data
      ...
      number of iterations, the convergence criteria (о), the iteration factor
      (з), and output options to print and/or punch the calculated values of
...
|TUBE   |Axial Compression (+2)    |     з/+s)         |Torsional Constant (J)|
|       |Torsion (з)               |                   |                      |
+-------+--------------------------+-------------------+----------------------+
|       |             +            |                   |                      |
...
|QDMEM  |Principal Compression (+2)|     зm/+s)        |(Thicknesses t1, t2,  |
|QDMEM1 |Maximum Shear (зm)        |                   |t3 for TRIM6)         |
|QDMEM2 |                          |                   |                      |
|IS2D8  |                          |                   |                      |
|TRIM6  |                          |                   |                      |
+-------+--------------------------+-------------------+----------------------+
...
|QUAD2  |                          |                   |                      |
+-------+--------------------------+-------------------+----------------------+
|SHEAR  |Maximum Shear (зm)        | зm                |Thickness (t)         |
|       |                          |----               |                      |
|       |                          | +s                |                      |
+-------+--------------------------+-------------------+----------------------+
...
In the theoretical aspects of the analysis, н is defined as the scalar
...
material, to a Biot-Savart field. It is н that is solved for by using the heat
transfer approach in NASTRAN.

In the anticipated applications of this method, accurate values of н will
...
least, to the point at which н may be considered small). These accuracy and
...
be as precise as desired because of the imposed н = 0 boundary condition.
...
of Laplace's equation in these coordinates is
            ...
            м   n                                 m      (Qmn(о))
н(о,ю,й) =  -   -  [A   cos (mй) + B   sin (mй)] P  (ю) [---------](10)
           n=0 m=0   mn             mn            n      (Qmn(оo))

where

н       = reduced magnetic scalar potential

о,ю,й   = prolate spheroidal coordinates

оo      = coordinate of the interior prolate spheroidal surface
...
P  ,Q   = Legendre functions of the first and second kind, respectively
 n   n
    ...
    +                      2+            +1
Amn | =  оm        (n-m)! ф  cos        ф
Bmn |    ---(2n+1) ------ |  sin (mй)dй | нo(ю,й)Pmn(ю) dю
    +    4+        (n+m)! х             х
                          0             -1
     +
     | 1, m = 0
...
нo(ю,й) = distribution of potential н on prolate spheroidal surface о=оo
...
analysis. A brief description of each card follows.
    ...
    5. The PERMBDY card specifies points on boundaries of dissimilar magnetic
       ...
       magnetic permeability ж, then fi at the point must be 0. (The integral
...
where
   ...
   [н] = matrix of eigenvectors (mode shapes), order n x m, with n = order of
...
where wa is the ath natural radian frequency.
   ...
   The matrix of loads is computed as follows: The matrix product [M][н] is of
order n x m and corresponds to the product Mi Xia of Equation 9. (Here, only
terms of [M][н] in the desired directions are used.) The ath column of [M][н]
...
static analyses of Step 7 may be replaced as follows: For the ath mode {нa},
...
responses of Equation 15. For the former, the matrix [MP] = [M][н] is input to
...
described are performed, except that matrix [PHIG] = [н] replaces [MP] and
...
4.    Computes [M][н] for later use.
5.    Computes [н][M][V] as described in Equation 2.
...
where
      ...
      {о}  =  mechanical strain components
      ...
      [e]  =  piezoelectric tensor
        ...
        S
      [о ] =  dielectric tensor evaluated at constant mechanical strain
...
axial directions, respectively, and н is the electric potential. The latter
...
azimuth position й. The Fourier series for the electric potential has the same
...
Note, however, that the program assumes that the electric potential нi is the
...
enter the full matrices [cE], [e], and [оS]. The only assumption concerning
these matrices is that [cE] and [оS] are symmetric.
...
the й-axis. Polarization direction 3 may vary in the R-Z plane and, for radial
polarization, coincides with the R-axis. When a user specifies properties on a
MATPZ1 card, the transformation from the 1, 2, 3 directions to the R, Z, й
...
that the ordering of the Z-й and R-Z shears is interchanged. Once again,
...
and are entered into {Fн} in Equation 4. Since the electric potential is
...
modeling and should prove helpful to you.
   ...
   5. The SE and d values on MATPZ1 cards will be multiplied by 10-12 by
      NASTRAN. Also, the value of о0 is fixed in NASTRAN as 8.854 x 10-12
      farad/meter.
   ...
   11.  Material properties specified on MATPZ1 cards are transformed by
        NASTRAN from the standard 1, 2, 3 material directions to the R, Z, й
        directions. Also, the transformation required due to a switch in the
        order of the R-Z and Z-й shears between conventional specifications
...
with its axis of rotation at an angle with respect to the tunnel mean flow.
      ...
      1.Vector к is the angular velocity of the XBYBZB (Basic) coordinate
...
represents the absolute inflow velocity, while AU (= к x RA) is the blade
...
IAV defines the relative inflow angle 8 (shown positive).
   The angle of sweep ю is defined as the angle of inclination of the chord BA
with the plane WAU. ю shown in Figure 1.20-2 is positive.

   AD is the projection of AC (BA extended to C) in the plane WAU. Angle lAD
represents the stagger angle ю, and is shown positive.
...
vector along y, for the sense of к shown in Figure 1.20-2, is given by
